...
class PostgreSQLAuthState {
    constructor(poolOrConfig, sessionId) {
        this.pool = poolOrConfig instanceof pg_1.Pool ? poolOrConfig : new pg_1.Pool(poolOrConfig);
                ...
                get: async (type, ids) => {
                    ...
                    await Promise.all(ids.map(async (id) => {
                        ...
                        if (type === 'app-state-sync-key' && value) {
                            data[id] = WAProto_1.proto.Message.AppStateSyncKeyData.fromObject(value);
                        }
                        else {
                            data[id] = value;
                        }
...
async function usePostgreSQLAuthState(poolOrConfig, sessionId) {
